...
# Define functions to be used in code

def freeflame_simulation(model, phi, T, P, o2_frac, domain, renormalize=False):
    ...
    gas.TPX = T, P, {'CH4': 1 * phi, 'O2': 2.0, 'N2': 7.52}
...
def main_rpd(modelname, flame, outfile_name, tracing_element='C', threshold=0.02):
    domain = flame.grid
    ...
    # Find the max flux
    max_flux = max_flux_calc(flx_max=0.0, int_rpd_data_list=flux_data)
...
# ########################################################################################################################
# # Main script
# model = 'ffcm1_fne_HCO_05atm_TROE_traject.yaml'
#
#
# # Output filename
# outfile = 'c2h4_air_phi1.0_p1atm_ffcm_fne.txt'
#
# # Reload pre-saved flame solution
# # filename_xml = 'CH4_25.0O2_phi0.7_t298K_p0.5atm_freeflame_Think.xml'
# # solution_title = 'CH4_25.0O2_phi0.7_t298K_p0.5atm_freeflame_model_Think'
# # flame_obj = flame_restore(model, filename_xml, solution_title)
# flame_obj = freeflame_simulation(model, 1.0, 298, 101325*1, 0.21, 0.03)
#
# main_rpd(model, flame_obj, outfile, threshold=0.02)
# # gas = ct.Solution(model)
# # think_molecules = molecule_list(gas, 'C')
